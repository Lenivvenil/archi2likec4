...
## V2: Исправления deployment-диаграмм

### V2-A1: Публикация в PyPI ✅

**Цель:** `pip install archi2likec4` / `pipx install archi2likec4` работают из PyPI.

**Что сделано:**
- `pyproject.toml` уже содержит все необходимые метаданные (name, version, classifiers, urls, entry_points)
- Создан `.github/workflows/publish.yml` — CI для публикации:
  - Триггер: GitHub Release (published)
  - Trusted publishing через OIDC (id-token: write), без API-ключей
  - Build: sdist + wheel через `python -m build`
  - Test matrix: Python 3.10–3.13 с `--cov-fail-under=80`
  - Двухэтапный publish: TestPyPI → PyPI (через GitHub Environments)
  - Артефакты сохраняются через `upload-artifact`

**Чтобы завершить настройку:**
1. На PyPI: зарегистрировать проект `archi2likec4`, настроить Trusted Publisher (GitHub OIDC)
2. На TestPyPI: аналогично
3. В GitHub repo Settings → Environments: создать `testpypi` и `pypi`
4. Создать GitHub Release с тегом `v1.0.0` — publish pipeline запустится автоматически
5. Проверить: `pipx install archi2likec4` и `archi2likec4 --help`

### V2-D1: infraSoftware shape: cylinder → rectangle ✅

**Проблема:** Все элементы infraSoftware отображались как цилиндры (dataStore shape),
хотя к персистентному хранению отношения не имеют. Визуальная путаница.

**Решение:** В `generate_spec()` изменён shape для infraSoftware с `cylinder` на `rectangle`.
Цилиндры остаются только для `dataStore` (16 элементов — все легитимные БД).

### V2-D2: Исключение appFunction из deployment views ✅

**Проблема:** На deployment solution views отображались функции (appFunction), хотя
для deployment-контекста достаточно уровня систем и подсистем.

**Решение:**
- `generators.py`: добавлен параметр `fn_archi_ids: set[str]` в `generate_solution_views()`.
  При view_type='deployment' archi_id из этого множества пропускаются.
- `pipeline.py`: собирается множество fn_archi_ids из всех функций всех систем/подсистем
  и передаётся в generate_solution_views().

### V2-D3: Вложенность приложений в инфраструктуру (open)

**Проблема:** Прикладные компоненты (подсистемы) отображаются РЯДОМ с техобъектами,
а должны быть визуально ВНУТРИ виртуалки/хоста, на котором стоят.

**Ограничение LikeC4:** Визуальная вложенность элементов определяется иерархией в модели.
App-элементы (domain → system → subsystem) и infra-элементы (infraNode → infraSoftware)
находятся в разных ветках модели, связаны через `deployedOn` relationships.
LikeC4 не поддерживает визуальную вложенность элементов из разных ветвей модели
в одном view — только стрелки.

**Варианты решения:**
1. LikeC4 deployment model (если/когда появится поддержка instance-of)
2. Nested extend: дублировать app-элементы внутри infra-узлов (плохо — дублирование)
3. Кастомные view с group-синтаксисом (ждём LikeC4 feature)
4. Текущее решение: deployedOn стрелки + app элементы на уровне system/subsystem

### V2-D4: Иерархия deployment topology

**Целевая иерархия:**
```
зона/площадка (infraLocation)
  → сетевая зона/сегмент (infraNode)
    → (alt1) baremetal сервер (infraNode)
    → (alt2) hypervisor (infraNode) → VM (infraNode)
      → системное ПО: ОС, middleware (infraSoftware)
      → ППО: элементы деплоя приложений (system/subsystem через deployedOn)
```

**Текущее состояние:** Иерархия строится автоматически из ArchiMate AggregationRelationship.
Если в исходной модели правильная вложенность (Location → Node → SystemSoftware), то
deployment topology корректна. Случай Kubernetes пока не адресован.

---

## V2: Code review fixes (2026-03-11)

### CR-1 [P1]: Занижение unresolved ratio для deployment views ✅

**Проблема:** `total_elements` считал все элементы включая appFunction, но appFunction
пропускались (`continue`) при подсчёте unresolved. Denominator рос искусственно.
Пример: 100 fn + 50 missing → лог `50/150 (33%)`, хотя реальный unresolved 100%.

**Исправление:** `generators.py` — фильтруем fn_archi_ids ДО подсчёта total_elements:
```python
...
### CR-2 [P1]: Глобальное состояние trash-фильтра между запусками ✅

**Проблема:** `pipeline.py:_parse()` мутировал `parsers.DEFAULT_TRASH_NAMES` без restore.
Для web-процесса это давало кросс-запросную «память» кастомного конфига.

**Исправление:** `pipeline.py` — save/restore в try/finally:
```python
_original = _parsers_mod.DEFAULT_TRASH_NAMES
if config.trash_folder_names is not None:
    _parsers_mod.DEFAULT_TRASH_NAMES = frozenset(config.trash_folder_names)
try:
    ...  # all parsing
finally:
    _parsers_mod.DEFAULT_TRASH_NAMES = _original
```

### CR-3 [P2]: CSRF-защита для Web UI — усиление ✅

**Было:** Origin/Referer check, но POST без обоих заголовков проходил (privacy-режимы).

**Исправление:** `web.py` — добавлен else-branch: POST без Origin и Referer → 403.
Тесты обновлены (Origin в redirect-тестах, новый тест `test_post_without_origin_or_referer_rejected`).

### CR-4 [P2]: Санитизация domain в /promote-system ✅

**Проблема:** `/assign-domain` вызывал `sanitize_path_segment()`, а `/promote-system` — нет.

**Исправление:** `web.py` — добавлен `sanitize_path_segment()` для domain в promote_system().

### CR-5 [P2]: Валидация C4 identifier в domain_renames ✅

**Проблема:** `new_id` в domain_renames не валидировался на формат LikeC4 identifier.
Можно было сгенерировать syntactically invalid `.c4`.

**Исправление:** `config.py` — regex `^[a-z_][a-z0-9_-]*$` для new_id.
Тесты: `test_domain_renames_invalid_c4_id`, `test_domain_renames_c4_id_starts_with_digit`.
